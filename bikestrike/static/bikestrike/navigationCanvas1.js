...
(function (root, factory) {
    ...
    'use strict';
    /**
     * class NavigationCanvasRenderer1 < Renderer
     *
     * Physics.renderer('navigationCanvasRenderer1')
     *
     * Renderer that uses HTMLCanvas to render the world bodies with viewport and borders.
     *
     ...
     *
     *  Use this class like simple canvas renedrer
     *
     **/
    Physics.renderer('navigationCanvas1', function( proto ){
        ...
        return {
            ...
            init: function( options ){
                ...
                if (viewport.nodeName.toUpperCase() !== 'CANVAS'){
                    ...
                    viewport.width = this.el.clientWidth;
                    viewport.height = this.el.clientHeight;
                ...
                this._layers = {};
                this.addLayer( 'main', this.el, options);
            ...
            addLayer: function( id, el, opts ){
                ...
                layer.render = function( clear ){
                    ...
                    if (layer.id == "main"){
                        bodies.forEach(function(item){
                            self._world.add(item);
                        });

                        // self._world._bodies = self._world._bodies.concat(bodies);
                        bodies = [];
                    }

                    stack = self._world._bodies;
                ...
                layer.createBoundRectangles = function(boundaries){
                  if (!boundaries.x || !boundaries.y
                    || !boundaries.viewportX || !boundaries.viewportY
                    || !boundaries.thickness)
                    throw "Boundaries object does not has all properties";

                  if (!boundaries.thickness)
                    boundaries.thickness = 50;

                  var left = Physics.body('rectangle', {
                      x: boundaries.thickness / 2.0,
                      y: boundaries.y / 2.0,
                      width: boundaries.thickness,
                      height: boundaries.y
                  });

                  var right = Physics.body('rectangle', {
                      x: boundaries.x - boundaries.thickness / 2.0,
                      y: boundaries.y / 2.0,
                      width: boundaries.thickness,
                      height: boundaries.y
                  });

                  var top = Physics.body('rectangle', {
                      x: boundaries.x / 2.0,
                      y: boundaries.thickness / 2.0,
                      width: -5 + boundaries.x - 2*boundaries.thickness,
                      height: boundaries.thickness
                  });

                  var bottom = Physics.body('rectangle', {
                      x: boundaries.x / 2.0,
                      y: boundaries.y - boundaries.thickness / 2.0,
                      width: -5 + boundaries.x - 2*boundaries.thickness,
                      height: boundaries.thickness
                  });

                  left.treatment = "static";
                  right.treatment = "static";
                  top.treatment = "static";
                  bottom.treatment = "static";

                  // debugger
                  bodies.unshift(left, top, right, bottom);
                  // self.getWorld()._bodies.unshift(left, top, right, bottom);
                }

                if (layer.options.bordered)
                {
                    layer.createBoundRectangles(
                      { x: layer.options.worldWidth || 4*layer.el.width, y: layer.options.worldHeight || 1.05*layer.el.height, viewportX: layer.el.width,  viewportY: layer.el.height, thickness: 50 }
                    )
                }
